...
给定一个肺部支气管CT扫描图像体$\mathbbm{X}_{airway}$，则支气管气道树分割任务可描述为\autoref{eq:seg_function}的函数关系，
...
为构建特征再学习，就是要定义一个对当前卷积层特征$\mathbbm{Feat}_m$的通道算子$\varPsi$。则对应的特征再学习(Feature Re-Learning, FRL)可定义为函数关系：
\begin{equation}
...
为了重新组合所有的通道，设计2个卷积层，如代码片段\ref{code:recombine_channels}所示，分别是将原来的通道$C_m$降低到$\frac{C_m}{r}$，然后将通道又恢复到$C_m$
\newpage
\begin{lstlisting}[
	language=Python,
...
基于前文对通道级特征再学习方法的研究，对3D-UNet基准网络进行改进，编程实现特征再学习模块的功能，在3D-UNet网络的每一个卷积层块后添加一个特征再学习模块。经过
...
集中验证集和测试集，计算它们的评价指标数据，如\autoref{tbl:comprehensive_metrics_overview}所示。
\begin{table}[ht]
    \centering
...
从\autoref{tbl:comprehensive_metrics_overview}挑选出9个有代表性的病例来显示支气管气道树三维模型，见\autoref{fig:airway_segmentation_overview}所示。
%\begin{table}[!ht]
%    \centering
...
%\subsection{消融研究}
%
%本文对肺部支气管气道树的分割采用3D-UNet网络作为基准分割模型，增加2个改进模块：注意力蒸馏模块和通道级特征再学习模块。前面三个章节分别展示了
%分割的效果和性能指标，作为比较，我们从性能指标角度研究它们的改进程度。如\autoref{tbl:perf_metrics_comparison}所示，
%\begin{table}[!htp]
%	\bicaption[3D-UNet基准模型，注意力蒸馏模块改进和特征再学习模块改进的性能指标对比]
%		{3D-UNet基准模型，注意力蒸馏模块改进和特征再学习模块改进的性能指标对比}
%		{The performance metrics comparison between 3D-UNet baseline, AD module improvement and FRL module improvement}
%	\label{tbl:perf_metrics_comparison}
%	\scalebox{0.9}{
%	\begin{tabular}{llccccccc}
%		\hline
%		&  &  FPR & FNR & Sensitivity & Precision & DSC & BD & TLD \\
%		\hline
%		3D-UNet baseline network & 平均值 & 0.031 &	6.815 & 	93.184 & 90.801 & 91.577 &	79.905 & 	87.677 \\
%		3D-UNet + AD             & 平均值 & 0.035 &	4.069 &	 95.930 &	89.956 &	92.758 &	85.997 &	 92.009 \\
%		3D-UNet + AD + FRL       & 平均值 & 0.032	& 3.736 &  	96.249 &	90.652 & 93.170 & 83.453 & 	90.883 \\
%		\hline
%		3D-UNet baseline network & 标准差 & 0.015	& 10.812 	& 10.812 	& 4.318 	& 7.228 	& 18.033	& 13.635 \\
%		3D-UNet + AD             & 标准差 & 0.016	& 2.684 	 & 2.684 	 & 4.252 	& 2.256  &	9.906 	 & 5.291 \\
%		3D-UNet + AD + FRL       & 标准差 & 0.016	& 1.883 	& 1.858 	& 4.542 & 2.270 	& 9.931 	& 4.926\\
%		\hline
%	\end{tabular}}
%\end{table}
%从性能指标的平均值来看，在FNR, Sensitivity, BD和TLD四个指标上，

\section{性能指标对比}

完成了综合实验之后，就性能指标与一些经典的网络结构进行对比。我们选择了V-Net\cite{Milletar2016VNetFC}, VoxResNet\cite{CHEN2018446}和
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|c|cccc}
        ...
        \hline
        \multicolumn{2}{c|}{\makecell{本文的方法\\3D-UNet + AD + FRL}} & 0.0328 & 93.171 & 83.454 & \uuline{\bf 90.883} \\
        \hline
        \hline
        ...
        \hline
        \hline
        \multicolumn{2}{c|}{\makecell{本文的方法在一些病例\\上取得的优秀表现\\以ATM\_260\_0000为例}} & 0.017 & \uuline\bf 96.57 & 97.24 & 96.57 \\
        \hline
    \end{tabular}
\end{table}
...
了解了通道级特征再学习的原理、计算过程和作用后，对3D-UNet网络基准基准模型进行改进，引入特征再学习模块，将其插入在每一层的卷积层块之后和最大池化层之间，
