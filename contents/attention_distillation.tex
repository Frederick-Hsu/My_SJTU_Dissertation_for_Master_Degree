...
引入注意力蒸馏方法后，可以来看看在卷积网络不同层之间，经过蒸馏后特征的表现如何？ 通过可视化的方式来查看支气管气道树的焦点
...
对于注意力蒸馏模块引入后额外增加的梯度，其带来的损失函数的计算，实现算法~\ref{algo:ad_loss}，
\begin{algorithm}[htb]
    \SetKwData{pred}{pred}
    ...
    \For{$i \leftarrow 0$ \KwTo \len{$AttMap$}-1 }{
    			\tcp{将第i个$AttMap$值赋给预测值}
        \pred $\leftarrow$ $AttMap[i]$		
        \BlankLine
        \tcp{将第i+1个$AttMap$值赋给再groundtruth}
        ...
        \BlankLine
        \tcp{预测值和真实值分别进行Softmax归一化}
        \predsoftmax $\leftarrow$ \softmax{\flattenpred, \dim}	
        \BlankLine
        \gtsoftmax $\leftarrow$ \softmax{\flattengt, \dim}		
        \BlankLine
        
        \tcp{计算它们的均方差，并作为当前循环的损失}
        \loss $\leftarrow$ \mseloss{\predsoftmax, \gtsoftmax}		
        \BlankLine
        
        \tcp{累积每次循环的损失，合并到总的损失}
        $Total\_Loss \leftarrow$ $Total\_Loss$ + $\gamma[i]$ * \loss		
    }
\end{algorithm}
需要累积各个卷积层的注意力蒸馏的梯度损失。骰子损失Dice Loss还是按照\autoref{eq:dice_loss}进行计算，Focal Loss损失按照\autoref{eq:focal_loss}计算，
最后需要将此计算出来的总的注意力蒸馏的梯度损失缀加到Total Loss（见\autoref{eq:total_loss}）上。需要指出的是，注意力蒸馏的梯度损失函数是可微可导的，因此在误差反向
传播时是可以更新3D-UNet网络的参数的。

\section{对比实验与实验结果分析}
对3D-UNet基准网络进行改进，加入了注意力蒸馏模块后，现在来进行一次实验，与前文的3D-UNet基准网络实验进行对比。使用的
数据集与3D-UNet实验的数据集完全相同，实验条件也是一致的。本次实验的训练、验证和测试三个阶段合计总耗时约39个小时。

验证集的指标数据如\autoref{tbl:3dunet_ad_metrics_valset}所示，表中每一项指标栏用双下划线加粗黑体标示出表现最优秀的, 
用波浪线标示出表现最差的。
\begin{table}[htb]
    \centering
    ...
    \begin{tabular}{cccccccc}
        ...
        ATM\_174\_0000\tnote{1} & 0.031 & 21.972 & 78.028 & 91.218 & 84.11 & 38.1   & 49.49  \\
        ATM\_215\_0000 & \uuline{\bf 0.021} & \uwave{10.595} & \uwave{89.405} & \uuline{\bf 92.453} & 90.9  & \uwave{72.25}  & 84.32  \\
        ATM\_505\_0000\tnote{1} & 0.061 & 44.617 & 55.383 & 85.209 & 67.13 & 26.51  & 42.18  \\
    ...
    \begin{tablenotes}
    	\footnotesize
    	\item[1] \textcolor{black}{ATM\_174\_0000与ATM\_505\_0000两个病例已在\autoref{tbl:validateset_airway_tree_metrics}分析过，它们是
    	存在严重肺部疾病的，支气管管壁破裂或缺失。基于此原因，将这两个病例排除在平均值的计算之外，但仍然在此表中展示出来。}
    \end{tablenotes}
    \end{threeparttable}
    }
\end{table}
...
测试集的指标数据如\autoref{tbl:3dunet_ad_metrics_testset}所示。从平均值来看，FPR、Sensitivity、DSC和TLD四项指标表现得都比较优秀。
\begin{table}[!htb]
    \centering
...
还有一个重要的实验结果就是支气管气道树分割效果可视化，分别从测试集和验证中挑选一些比较有代表性的病例来展示它们的分割效果，如图
...
\begin{figure}[!htp]
    ...
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_060_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_060\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_074_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_074\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_245_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_245\_0000}
    \end{subfigure}
    \\
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_246_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_246\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_260_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_260\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_266_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_266\_0000}
    \end{subfigure}
    \\
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_271_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_271\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/test040/ATM_638_0000_airway_tree_with_3colors_at_test_epoch40}
        \caption{ATM\_638\_0000}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.325\textwidth}
        \includegraphics[width=\textwidth]{results/baseline_ds_ad/val040/ATM_688_0000_airway_tree_with_3colors_at_val_epoch40}
...
改进网络进行对比实验，采用与3D-UNet网络基准模型同样的数据集和实验条件，完成训练、验证和测试全过程。从评价指标来看，3D-UNet + AD网络
