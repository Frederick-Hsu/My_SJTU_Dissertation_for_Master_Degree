...
CT扫描图像在$H3 \times W3$窗口内的Hounsfield unit\cite{HU2016CT}灰度
%\footnote{Hounsfield unit(亨氏单位)
...
，相应地像素矩阵也同步扩展到三维。三维卷积的操作如\autoref{fig:3D_Conv}所阐述。
\begin{figure}[!htp]
    \centering
...
\begin{table}[!ht]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \bicaption[3D-UNet下采样路径的卷积层块和池化层的结构]{3D-UNet下采样路径的卷积层块和池化层的结构}
            {The convolution block and pooling layer in the down-samping path of 3D-UNet}
        \label{tbl:down_sampling}
        {\ttfamily
            \begin{tabular}{c}
                Input(feature\_channel=n) \\
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{
                \makecell{Conv3d(in\_channel=n, \\
                                 out\_channel=n, \\
                                 kernel\_size=3, \\
                                 stride=1, \\
                                 padding=1)} 
                ...
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{InstanceNorm3d(num\_features=n)} \\ \hline
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{ReLU(inplace=True)} \\ \hline
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{
                \makecell{Conv3d(in\_channels=n, \\
                                 out\_channels=2n, \\
                                 kernel\_size=3, \\
                                 stide=1, \\
                                 padding=1)}
                ...
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{InstanceNorm3d(num\_features=2n)} \\ \hline
                $\Downarrow$ \\ \hline
                \multicolumn{1}{|c|}{ReLU(inplace=True)} \\ \hline
                $\Downarrow$ \\
                Output(feature\_channel=2n) \\
                \\ \\ \hline
                \multicolumn{1}{|c|}{
                \makecell{MaxPool3d(kernel\_size=(2, 2, 2), \\
                                    stride=(2, 2, 2), \\
                                    padding=0, \\
                                    dilation=1, \\
                                    ceil\_mode=False)}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \bicaption[3D-UNet上采样路径的反卷积层块和池化层的结构]{3D-UNet上采样路径的反卷积层块和池化层的结构}
            {The deconvolution block and pooling layer in the up-sampling path of 3D-UNet}
        \label{tbl:up_sampling}
        {\ttfamily
            \begin{tabular}{c}
                \hline
                \multicolumn{1}{|c|}{
                \makecell{Upsample(scale\_factor=2.0, \\
                                   mode='nearest')}
                } \\ \hline
                \\ \\ 
                Output(feature\_channel=$\frac{n}{2}$) \\
                $\Uparrow$ \\ \hline
                \multicolumn{1}{|c|}{ReLU(inplace=True)} \\ \hline
                $\Uparrow$ \\ \hline
                \multicolumn{1}{|c|}{InstanceNorm3d(num\_features=$\frac{n}{2}$)} \\ \hline
                $\Uparrow$ \\ \hline
                \multicolumn{1}{|c|}{\makecell{Conv3d(in\_channels=$\frac{n}{2}$, \\
                                                      out\_channels=$\frac{n}{2}$, \\
                                                      kernel\_size=$\frac{n}{2}$, \\
                                                      stride=1, \\
                                                      padding=1)}} \\ \hline
                $\Uparrow$ \\ \hline
                \multicolumn{1}{|c|}{ReLU(inplace=True)} \\ \hline
                ...
                \multicolumn{1}{|c|}{InstanceNorm3d(num\_features=$\frac{n}{2}$)} \\ \hline
                $\Uparrow$ \\ \hline
                \multicolumn{1}{|c|}{\makecell{Conv3d(in\_channels=n, \\
                                                      out\_channels=$\frac{n}{2}$, \\
                                                      kernel\_size=3, \\
                                                      stride=1, \\
                                                      padding=1)}} \\ \hline
...
3D-UNet主干网络包括编码器Encoder和解码器Decoder两大部分，编码器主要用来实现图像的特征提取，扩大感受野，输出具有
...
\begin{figure}[!htp]
    ...
    \bicaption[AI超算平台Slurm作业调度系统]
        {AI超算平台Slurm作业调度系统\cite{HpcSlurm2022}}
...
为了增强训练效果，也是为了大幅扩大数据量，对每个切割的长方体子块图像块进行了数据增强，具体的数据增强方式有：
...
如何计算这些评价指标，以模型预测的支气管体素对比真实的支气管体素的比对\autoref{tbl:metrics_calculation}来说明，
并定义计算方法。
\begin{table}[!htp]
...
经过模型训练后分割出来的支气管气道树，可以将其导入到ITK-SNAP\cite{Yushkevich2006ITKSNAP}软件中显示分割效果。这里
选择ATM\_054\_0000这个病例来可视化支气管气道树分割3D模型（这是经过3D-UNet基准网络分割的结果），如图
\ref{fig:airway_tree_model}所示。
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{visualize_colored_airway_tree}
...
在训练过程中使用PyTorch提供的TensorBoard功能收集每一个迭代周期Epoch的损失函数值，并密切注视着损失函数的曲线。
...
\autoref{tbl:time_consumption}反映了整个实验过程。经历了60个迭代周期训练过程完成。在第61个迭代周期，使用训练获得的最新模型
...
我们将进行横向和纵向的对比。
\begin{enumerate}
    \item[A.] 在横向对比时，对验证集的9例CT图像，每例CT图像均匀地取5张切片，选取中间的那张切片。排列这9张
切片看看它们的分割效果和指标数据，如\autoref{tbl:hcomparison_metrics}所示。
    ...
    需要说明的是，\autoref{tbl:hcomparison_metrics}中的指标数据是基于当前切片计算的，不是基于整个气道树来计算的，因此就不
    带有BD和TLD两个指标的数据了。还有一点，切片图像中由绿色像素框起来的区域是真实的气管，红色的像素是分割出来的。绿色像素覆盖
    在红色像素上，对于没有覆盖住而露出来的红色像素是假阳性的。
    
    \item[B.] 在纵向对比时，挑选ATM\_054\_0000这个病例，将第10个迭代周期，第20/30/40/50/60个迭代周期里的Slice
    \#264拿出来对比，看看它们的分割效果和指标数据，如\autoref{tbl:vcomparison_metrics}所示。
    \begin{table}[ht]
...
最后来看看3D-UNet网络对整个支气管气道树的分割表现。对于测试集的19例CT图像的分割，其性能指标如表
...
对于验证集的9例CT图像，不仅要展示支气管气道树分割的可视化结果，如\autoref{fig:visualize_airway_3d_model}所示。
%\begin{table}[!htp]
...
\begin{figure}[!htp]
	...
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_029_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_029\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_054_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_054\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_055_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_055\_0000}
	\end{subfigure}
	\\
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_057_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_057\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_091_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_091\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_174_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_174\_0000}
	\end{subfigure}
	\\
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_215_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_215\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_505_0000_airway_tree_with_3colors_at_val_epoch1}
		\caption{ATM\_505\_0000}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.325\textwidth}
		\includegraphics[width=\textwidth]{results/baseline/validate/ATM_688_0000_airway_tree_with_3colors_at_val_epoch1}
...
\end{figure}
还有完整的指标数据，见\autoref{tbl:validateset_airway_tree_metrics}。
\begin{table}[!htp]
...
体素。结合\autoref{tbl:validateset_airway_tree_metrics}中这两个病例的BD、TLD两项指标低至30\%的不正常现象\footnote{
